...
(defn- dgNode
 ...
 (if (:type iri)
  ...
  (assoc (co/IRI iri) :type :dgNode :innerType :dgNode)))
...
(defn mainClasses 
 "MainClasses ::= ‘MainClasses’ ‘(’ Class { Class } ‘)’"
 ([class]
  (-mainClasses #{(ex/class class)}))
 ([class & classes]
  (-mainClasses (into #{} (conj (if (= (type classes) clojure.lang.ArraySeq) (apply map ex/class classes)(map ex/class classes)) (ex/class class))))))
