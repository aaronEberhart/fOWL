...
(defn- -axiom 
 "Axiom := Declaration | ClassAxiom | ObjectPropertyAxiom | DataPropertyAxiom | DatatypeDefinition | HasKey | Assertion | AnnotationAxiom | DGAxiom"
 [axiom]
...
(defn- -dgRule
 ...
 ([body head]
  (if (and (= (:type body) :body) (every? (fn [x] (or (= (:innerType x) :classAtom)(= (:innerType x) :roleAtom)) (:atoms body))))
   (if (and (= (:type head) :head) (every? (fn [x] (or (= (:innerType x) :classAtom)(= (:innerType x) :roleAtom)) (:atoms head))))
  ...
  (if (and (= (:type body) :body) (every? (fn [x] (or (= (:innerType x) :classAtom)(= (:innerType x) :roleAtom)) (:atoms body))))
   (if (and (= (:type head) :head) (every? (fn [x] (or (= (:innerType x) :classAtom)(= (:innerType x) :roleAtom)) (:atoms head))))
    (if (= (:type annotations) :axiomAnnotations)
     {:annotations (:annotations annotations) :body (:atoms body) :head (:atoms head) :type :dgRule :innerType :dgRule :outerType :dgRule}
...
(defn dgRule
 "DGRule ::= DescriptionGraphRule ‘(’ {Annotation} ‘Body’ ‘(’ {DGAtom} ‘)’ ‘Head’ ‘(’ {DGAtom} ‘)’ ‘)'"
 ([body head]
  (-axiom (-rule (-dgRule body head))))
 ([annotations body head]
  (-axiom (-rule (-dgRule (ann/axiomAnnotations annotations) body head)))))
...
(defn- -dgAxiom 
 ...
 ([dgname dgnodes dgedges mainclasses]
  (if (= (:type dgname) :dgName)
   (if (= (:type dgnodes) :dgNodes)
    (if (= (:type dgedges) :dgEdges)
     (if (= (:type mainclasses) :mainClasses)
      {:dgName dgname :dgNodes (:nodes dgnodes) :dgEdges (:edges dgedges) :mainClasses (:classes mainclasses) :type :dgAxiom :innerType :dgAxiom :outerType :dgAxiom}
      (throw (Exception. (str {:type ::notMainClasses :MainClasses mainclasses}))))
     (throw (Exception. (str {:type ::notDGEdges :Edges dgedges}))))
    (throw (Exception. (str {:type ::notDGNodes :Noes dgnodes}))))
   (throw (Exception. (str {:type ::notDGName :Name dgname})))))
 ([annotations dgname dgnodes dgedges mainclasses]
  (if (= (:type dgname) :dgName)
   (if (= (:type dgnodes) :dgNodes)
    (if (= (:type dgedges) :dgEdges)
     (if (= (:type mainclasses) :mainClasses)
      (if (= (:type annotations) :axiomAnnotations)
       {:annotations (:annotations annotations) :dgName dgname :dgNodes (:nodes dgnodes) :dgEdges (:edges dgedges) :mainClasses (:classes mainclasses) :type :dgAxiom :innerType :dgAxiom :outerType :dgAxiom}
       (throw (Exception. (str { :type ::notAnnotations :Annotations annotations}))))
      (throw (Exception. (str {:type ::notMainClasses :MainClasses mainclasses}))))
     (throw (Exception. (str {:type ::notDGEdges :Edges dgedges}))))
    (throw (Exception. (str {:type ::notDGNodes :Noes dgnodes}))))
...
(defn dgAxiom 
 "DGAxiom ::= ‘DescriptionGraph’ ‘(’ {Annotation} DGName DGNodes DGEdges MainClasses ‘)’"
 ([dgname dgnodes dgedges mainclasses]
  (-axiom (-dgAxiom dgname dgnodes dgedges mainclasses)))
 ([annotations dgname dgnodes dgedges mainclasses]
  (-axiom (-dgAxiom (ann/axiomAnnotations annotations) dgname dgnodes dgedges mainclasses))))
...
(defn roleChain
 "propertyExpressionChain := 'ObjectPropertyChain' '(' ObjectPropertyExpression ObjectPropertyExpression { ObjectPropertyExpression } ')'"
 ([role1 role2 & roles]
  (-roleChain (into [] (conj (if (= (type roles) clojure.lang.ArraySeq) (apply map ex/role roles) (map ex/role roles))(ex/role role1)(ex/role role2)))))
 ([role1 role2]
